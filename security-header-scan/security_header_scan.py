# security_header_scan.py - Security Headers Analyzer for Burp Suite
# Production-ready Jython extension for comprehensive security header analysis
# Supports policy-driven scoring, URL relevance filtering, and risk assessment

from burp import IBurpExtender, ITab, IHttpListener, IContextMenuFactory, IMessageEditorTabFactory
from burp import IContextMenuInvocation
from java.awt import BorderLayout, FlowLayout, GridLayout, Font, Color
from java.awt.event import ActionListener, MouseListener
from java.util import ArrayList
from javax.swing import (
    JPanel, JFrame, JLabel, JButton, JCheckBox, JComboBox, JTable, 
    JScrollPane, JTabbedPane, JSplitPane, JTextArea, JFileChooser,
    DefaultComboBoxModel, JPopupMenu, JMenuItem, SwingUtilities, JOptionPane,
    table, BorderFactory, JTextField, Box, BoxLayout
)
from javax.swing.filechooser import FileNameExtensionFilter
from javax.swing.table import DefaultTableModel, AbstractTableModel
from javax.swing.event import MouseAdapter
...
SECURITY_HEADERS = {
    'Strict-Transport-Security': {
        ...
        'critical': True,
        'patterns': {
            'strong': r'max-age=\d{6,}.*includeSubDomains.*preload',
            'medium': r'max-age=\d{6,}',
            'weak': r'max-age=\d{1,5}'
        }
    },
    'Content-Security-Policy': {
        ...
        'critical': True,
        'patterns': {
            'strong': r"default-src\s+'self'",
            'medium': r'default-src',
            'weak': r"'unsafe-inline'|'unsafe-eval'"
        }
    },
    'X-Frame-Options': {
        'weight': 20,
    ...
    'Referrer-Policy': {
        ...
        'critical': False,
        'patterns': {
            'strong': r'no-referrer|strict-origin-when-cross-origin',
            'medium': r'same-origin'
        }
    },
    'Permissions-Policy': {
        'weight': 8,
    ...
    'Cache-Control': {
        ...
        'critical': False,
        'patterns': {
            'strong': r'no-store.*no-cache',
            'medium': r'no-cache|no-store',
            'weak': r'public'
        }
    }
}
...
APPLICATION_PROFILES = {
    'SPA': {
        ...
        'weight_csp': 1.5,
        'weight_hsts': 1.2,
        'weight_clickjack': 1.3
    },
    'REST_API': {
        'name': 'REST API',
        'weight_csp': 0.8,
        'weight_hsts': 1.0,
        'weight_clickjack': 0.9
    },
    'TRADITIONAL_WEB': {
        'name': 'Traditional Web Application',
        'weight_csp': 1.2,
        'weight_hsts': 1.0,
        'weight_clickjack': 1.2
    },
    'ADMIN_PORTAL': {
        'name': 'Admin Portal',
        'weight_csp': 1.5,
        'weight_hsts': 1.5,
        'weight_clickjack': 1.5
    }
}

STATIC_EXTENSIONS = {
    '.css', '.js', '.png', '.jpg', '.jpeg', '.gif', '.ico', '.woff', '.woff2',
    '.ttf', '.eot', '.svg', '.mp3', '.mp4', '.webm', '.pdf', '.zip', '.tar'
}

STATIC_KEYWORDS = {
    'cdn', 'static', 'assets', 'dist', 'public', 'images', 'fonts', 'media',
    'vendor', 'lib', 'bundle', 'chunk', 'resource'
}
...
PROFILES = {
    "Default Web Application": {
        "Strict-Transport-Security": 25, "Content-Security-Policy": 30,
        "X-Frame-Options": 20, "X-Content-Type-Options": 15,
        "Referrer-Policy": 10, "Permissions-Policy": 8, "Cache-Control": 12,
...
class SecurityHeaderScanner(IBurpExtender, ITab, IHttpListener, IContextMenuFactory):
    """
    ...
    def processHttpMessage(self, toolFlag, messageIsRequest, messageInfo):
        ...
        if self._show_excluded_checkbox.isSelected() and not messageIsRequest and self._callbacks.isInScope(self._helpers.analyzeRequest(messageInfo).getUrl()):
            ...
            if score < 40:
                with self._lock:
                    self._excluded_urls.add(messageInfo)
            ...
            def add_hosts_action(e):
                try:
                    ...
                    for msg in selected_messages:
                        ...
                        host = "{}://{}".format(url.getProtocol(), url.getHost())
                        hosts.add(host)
                    
                    for host in hosts:
                        if host not in self._scope_list_model:
                            self._scope_list_model.addElement(host)
                    
                    JOptionPane.showMessageDialog(None, "Added {} host(s) to the Security Headers Analyzer.".format(len(hosts)), "Success", JOptionPane.INFORMATION_MESSAGE)
                except Exception as ex:
                    JOptionPane.showMessageDialog(None, "Error adding hosts: {}".format(str(ex)), "Error", JOptionPane.ERROR_MESSAGE)
        ...
        try:
            ...
            if sitemap:
                for item in sitemap:
                    ...
                    url_str = "{}://{}".format(url.getProtocol(), url.getHost())
                    if url_str in selected_hosts:
            ...
            for messageInfo in requests_to_analyze:
                ...
                if self._auto_detect_checkbox.isSelected():
                    relevance_score, reason = self._get_relevance_score(messageInfo)
                    if relevance_score < 40:
                        continue
                
                result = self._analyze_single_request(messageInfo, app_context)
                self._analyzed_results.add(result)
            ...
            def update_table():
                ...
                if self._analyzed_results.isEmpty() and len(requests_to_analyze) > 0:
                    JOptionPane.showMessageDialog(self._main_panel, "All requests were filtered out. Try disabling 'Auto-detect meaningful URLs'.", "No Results", JOptionPane.INFORMATION_MESSAGE)
    ...
    def _analyze_single_request(self, messageInfo, app_context):
        ...
        for header in headers:
            if ":" in header:
                ...
                header_map[key.lower().strip()].append(value.strip())
    ...
    def _export_as_csv(self, file_path):
        with open(file_path, 'wb') as f:
            ...
            # Write summary
            writer.writerow(["Overall Score", self._overall_score_label.getText()])
            writer.writerow(["Overall Risk", self._overall_risk_label.getText()])
            ...
            # Write data
            for result in self._analyzed_results:
                row = [str(result.get("URL")), str(result.get("Score")), str(result.get("Risk"))]
                for header in headers[3:]:
                    row.append(str(result.get("headers", {}).get(header, "N/A")))
                writer.writerow(row)
    ...
    def _export_as_json(self, file_path):
        summary = {
            "overallScore": self._overall_score_label.getText(),
            "overallRisk": self._overall_risk_label.getText(),
            "applicationContext": {
                "exposure": self._exposure_dropdown.getSelectedItem(),
                "auth": self._auth_dropdown.getSelectedItem(),
...
class ResultsTableModel(AbstractTableModel):
    ...
    def __init__(self, data):
        self._data = data
        self._column_names = ["URL", "Score", "Risk", "Strict-Transport-Security", "Content-Security-Policy", 
